    ...
    with col_up:
        ...
        if files:
            ...
            for col, f in zip(tcols, files[:5]):
                with col:
                    f.seek(0)
                    st.image(Image.open(f), width="stretch")
        ...
        if st.session_state.wardrobe_items:
            hr()
            if st.button("🗑️  Clear wardrobe & start over", width="stretch"):
                try:
                ...
                with st.expander(f"✓  {fname}  ·  {n} item{'s' if n!=1 else ''}", expanded=(n>0)):
                    ...
                    if res["items"]:
                        ...
                        for col, it in zip(ic, res["items"][:3]):
                            with col:
                                if it.get("crop_b64"):
                                    st.image(b64_to_pil(it["crop_b64"]), width="stretch")
                                item_card(it)
                        if n > 3:
...
with tab3:
    ...
    if not st.session_state.wardrobe_items:
        ...
        with fc:
            ...
            get_btn = st.button("✨  Get My Outfit", width="stretch", disabled=not can_rec)
